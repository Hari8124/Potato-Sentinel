...
def predict_image(img_stream):
    try:
        ...
        logger.debug("Starting prediction using CNN model features extraction...")
        cnn_features = img_array.flatten().reshape(1, -1)  # Flatten image array to extract raw features
        logger.debug(f"Completed CNN feature extraction. Shape: {cnn_features.shape}")

        # Step 3: Scale features using the scaler
        logger.debug("Starting feature scaling with scaler for Random Forest model...")
        rf_features = scaler.transform(cnn_features)
        logger.debug(f"Completed feature scaling. Scaled features shape: {rf_features.shape}")
